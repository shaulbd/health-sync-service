	...
	PluginFactory pluginFactory) : BackgroundService
{
	...
	/// <param name="stoppingToken"></param>
	/// <returns></returns>
	public async Task<SyncResult> ExecuteSyncTaskAsync(SyncTask task, CancellationToken stoppingToken = default)
	{
	...
	/// <param name="stoppingToken"></param>
	/// <returns></returns>
	public async Task<SyncResult> ExecuteSyncTaskAsync(string taskId, DateTimeOffset? start, DateTimeOffset? end, CancellationToken stoppingToken = default)
	{
		var task = FindTask(taskId);
		if (task == null)
		{
			return new SyncResult(SyncResultStatus.TaskNotFound, $"Task with ID: {taskId} not found.");
		}

		return await ExecuteSyncTaskAsync(task with { Start = start, End = end }, stoppingToken);
	}

	/// <summary>
	/// Executes a last sync search for a tasks repository plugin and returns the result.
	/// </summary>
	/// <param name="taskId">Task unique id</param>
	/// <param name="stoppingToken"></param>
	/// <returns></returns>
	public async Task<DateTimeOffset?> GetLastSync(string taskId, CancellationToken stoppingToken = default)
	{
		var task = FindTask(taskId);

		_logger.LogDebug("Creating repository plugins");
		var syncContext = SyncContext.Create(task);
	...
	private async Task<SyncResult> ProcessSyncRanges(List<(DateTimeOffset Start, DateTimeOffset End)> ranges, SyncTask task, IProviderPlugin providerPlugin, IRepositoryPlugin repositoryPlugin, CancellationToken stoppingToken)
	{
		...
		for (var i = 0; i < ranges.Count; i++)
		{
			...
			try
			{
				await ProcessSyncChunk(task, providerPlugin, repositoryPlugin, start.NoFractions(), end.NoFractions(), part, stoppingToken);
	...
	private async Task ProcessSyncChunk(SyncTask task, IProviderPlugin providerPlugin, IRepositoryPlugin repositoryPlugin, DateTimeOffset start, DateTimeOffset end, string part, CancellationToken stoppingToken)
	{
		...
		var healthData = await new Func<Task<HealthData>>(() =>
			providerPlugin.GetAsync(start, end, task.Index, stoppingToken)
		).WithRetry(_configuration.RetryCount, logger);

		_logger.LogDebug("({Part}) Pushing health data to repository...", part);
		await new Func<Task>(() =>
			repositoryPlugin.PushAsync(task.Index, healthData, stoppingToken)
		).WithRetry(_configuration.RetryCount, logger);
	...
	/// <summary>
	/// Find task in configuration.
	/// </summary>
	/// <param name="taskId">Task ID.</param>
	/// <returns></returns>
	/// <exception cref="KeyNotFoundException"></exception>
	private SyncTask FindTask(string taskId)
	{
		var task = _configuration.Sync.FirstOrDefault(s => s.Id.Equals(taskId));
		return task;
	}
}